...
if CLIENT then
   ...
   SWEP.EquipMenuData = {
      type = "item_weapon",
      desc = [[
Rapidly fires a wall of bullets with
very low damage but very high push force.

Can be charged up with secondary fire.]]
   };
...
function SWEP:FirePulse(force_fwd, force_up, charge)
   ...
   sound.Play(self.Primary.Sound, self:GetPos(), self.Primary.SoundLevel, 80 - charge * 20)

   self:SendWeaponAnim(ACT_VM_IDLE)

   local cone = Lerp(charge, self.Primary.Cone, 0.07)
   local num = 25 + math.floor(charge * 25)
...
function SWEP:ChargedAttack()
   ...
   self:FirePulse(force_fwd, force_up, charge)
end
...
if CLIENT then
   ...
   function SWEP:DrawHUD()
      ...
      local charge = math.Clamp(self.dt.charge, 0, 1)
      ...
      if LocalPlayer():IsTraitor() then
		...
		local xhairRadius = Lerp(charge, 0.22, 0.035)
		surface.DrawCircle(ScrW() * 0.5, ScrH() * 0.5, ScrH() * xhairRadius, LocalPlayer():IsTraitor() and 255 or 0, LocalPlayer():IsTraitor() and 0 or 255, 0)
...
--[[
      if nxt < CurTime() or CurTime() % 0.5 < 0.2 or charge > 0 then
		...
		-- new crosshair is just a circle
      end
]]
